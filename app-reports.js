function togglePanel(panelName) {
  ...
  ['feedback', 'report', 'settings', 'soundscape', 'language', 'boost', 'music', 'sync', 'theme', 'calendar-dropdown', 'inspiration', 'shopping', 'cooking', 'alarm', 'weather', 'news', 'pause-dropdown', 'logo-guide'].forEach(p => {
...
function updateReportPanel() {
      ...
      if (totalInCat > 0) {
        ...
        row.innerHTML = `
          <div class="flex justify-between items-center text-[11px]">
            ...
            <span class="text-gray-400 font-mono">${completedInCat} von ${totalInCat}</span>
          </div>
  ...
  if (insightEl) {
    if (count === 0) {
      insightEl.innerText = tr({
        de: 'Noch keine Aufgaben abgeschlossen. Starte mit einer kleinen Aufgabe!',
        en: 'No tasks completed yet. Start with a quick win!'
      });
    } else if (count < 4) {
      insightEl.innerText = tr({
        de: `Guter Fortschritt! Du hast bereits ${count} Aufgabe(n) gemeistert. Bleib dran!`,
        en: `Good progress! You already completed ${count} task(s). Keep going!`
      });
    } else if (count < 8) {
      insightEl.innerText = tr({
        de: `Starke Leistung! Schon ${count} Aufgaben erledigt. Dein Rhythmus stimmt!`,
        en: `Great performance! ${count} tasks completed. You are in flow!`
      });
    } else {
      insightEl.innerText = tr({
        de: `Hervorragend! ${count} Aufgaben geschafft – ein extrem produktiver Zeitraum! 🚀`,
        en: `Outstanding! ${count} tasks completed – an extremely productive period! 🚀`
      });
    }
  }
  ...
  if (list) {
    ...
    if (filteredDone.length === 0) {
      list.innerHTML = `<div class="text-gray-500 italic text-center py-2 text-xs">${tr({ de: 'Keine Protokolleinträge vorhanden.', en: 'No logs available.' })}</div>`;
    } else {
      filteredDone.slice().reverse().forEach(item => {
        ...
        div.innerHTML = `
          <div class="flex items-center gap-1.5 overflow-hidden pr-2">
            ...
            <span class="truncate font-semibold text-xs text-white">${item.task}</span>
          </div>
          <span class="text-gray-500 font-mono text-[10px] shrink-0">${item.time || item.date || ''}</span>
        `;
