      ...
      grouped[deptKey].forEach(({ item, originalIdx }) => {
        ...
        html += `
          <div class="flex items-center justify-between gap-1.5 p-1.5 bg-black/40 hover:bg-white/[0.04] border border-white/5 rounded-xl text-gray-300 transition group">
            ...
            <span class="truncate font-medium flex-1 pl-1 text-xs text-white" title="${item.name}">${qtyLabel}${item.name}</span>
  ...
  if (historyList) {
    ...
    if (hist.length === 0) {
      ...
      hist.slice().reverse().forEach((hItem, hIdx) => {
        ...
        hDiv.innerHTML = `
          <span class="truncate max-w-[140px] line-through text-gray-400 font-medium">${hItem.name}</span>
