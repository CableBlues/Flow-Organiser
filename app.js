...
// DYNAMIC TRANSLATION TRANSLATOR ENGINE
...
// Dynamically translates unchanged standard lists inside State on language change
function translateUserTasks(fromLang, toLang) {
  ...
  // Map active task in background steps state too
...
// Resolves a localized standard task back to its German key for task step matching
...
function formatTerminDate(dateStr, timeStr) {
  ...
  if (dateStr === todayISO) {
    ...
    label = localizedTomorrow;
  } else {
    const parts = dateStr.split('-');
    if (parts.length === 3) {
      label = `🗓️ ${parts[2]}.${parts[1]}.`;
    } else {
      label = `🗓️ ${dateStr}`;
    }
  }
...
function renderApp() {
  ...
  CATEGORIES.forEach(([id, iconKey]) => {
    ...
    let titleText = t(id);
    if (isDone) {
      titleText += ` (${state.done.length})`;
    } else if (!isNotes) {
      titleText += ` (${doneInCat}/${totalInCat})`;
    }
    ...
    if (isDone) {
      state.done.slice().reverse().forEach((item, idx) => {
        ...
        itemDiv.title = 'Restore Task';
        itemDiv.onclick = () => handleRestoreDoneTask(idx);
        itemDiv.innerHTML = `
          <span class="truncate">${item.task} · ${item.time}</span>
          <i data-lucide="undo" class="w-3.5 h-3.5 opacity-0 group-hover:opacity-100 text-purple-400 shrink-0"></i>
        `;
      ...
      textarea.oninput = (e) => {
        state.items.notes = e.target.value;
        saveState();
      };
      listEl.appendChild(textarea);
    } else if (isTermine) {
      // Render sorted appointments chronologically by date & time
        ...
        itemDiv.innerHTML = `
          ...
          ${dateBadge ? `
            <div class="flex items-center mt-0.5">
              <span class="text-[10px] px-1.5 py-0.5 rounded ${isToday ? 'bg-amber-400/20 text-amber-300 border border-amber-400/30 font-bold' : 'bg-white/5 text-gray-300 border border-white/10'} font-mono">${dateBadge}</span>
            </div>
          ` : ''}
        `;
        listEl.appendChild(itemDiv);
      ...
      // Smart on-demand form for Termine
      if (!isTerminFormOpen) {
        ...
        btnEl.innerHTML = `
          <i data-lucide="calendar-plus" class="w-3.5 h-3.5 text-[var(--accent-light)]"></i>
          <span>${btnT}</span>
        `;
        ...
        const timeT = { de: 'Uhrzeit', en: 'Time', es: 'Hora', el: 'Ώρα' }[currentLang] || 'Time';
        ...
        formDiv.innerHTML = `
          <div class="flex items-center justify-between text-xs font-bold text-amber-300">
            ...
            <button onclick="toggleTerminForm(false)" class="text-gray-400 hover:text-white p-0.5 cursor-pointer text-xs" title="Close">✕</button>
          </div>
          ...
          <div class="grid grid-cols-2 gap-2">
            <div>
              <label class="text-[10px] text-gray-400 mb-0.5 block font-medium">${dateT}</label>
              <input type="date" id="add-termin-date" value="${todayISO}" class="w-full p-1.5 bg-black/60 border border-white/15 rounded-lg text-xs text-gray-200 outline-none focus:border-[var(--accent)] cursor-pointer" />
            </div>
            <div>
              <label class="text-[10px] text-gray-400 mb-0.5 block font-medium">${timeT}</label>
              <input type="time" id="add-termin-time" value="10:00" class="w-full p-1.5 bg-black/60 border border-white/15 rounded-lg text-xs text-gray-200 outline-none focus:border-[var(--accent)] cursor-pointer" />
            </div>
          </div>
          <div class="flex items-center gap-2 mt-1">
            <button onclick="handleAddTermin()" class="flex-1 py-1.5 bg-[var(--accent)] hover:opacity-90 text-white font-bold text-xs rounded-lg transition cursor-pointer flex items-center justify-center gap-1.5 shadow-sm">
              <i data-lucide="check" class="w-3.5 h-3.5"></i>
              <span>${saveT}</span>
            </button>
            <button onclick="toggleTerminForm(false)" class="px-3 py-1.5 bg-white/5 hover:bg-white/10 text-gray-300 font-semibold text-xs rounded-lg transition cursor-pointer">
              ${cancelT}
            </button>
          </div>
        `;
      ...
      (state.items[id] || []).forEach((task, index) => {
        ...
        itemDiv.innerHTML = `
          ...
          <div class="absolute right-1.5 top-1/2 -translate-y-1/2 flex items-center gap-1 opacity-0 group-hover:opacity-100 pointer-events-none group-hover:pointer-events-auto transition duration-150 shrink-0 bg-[#13131a]/95 border border-white/15 px-1 py-0.5 rounded-lg shadow-lg z-10">
            ...
            <button onclick="startTaskTimer('${safeTaskEscaped}', event)" class="p-1 text-amber-400 hover:text-amber-300 hover:bg-amber-500/10 rounded transition cursor-pointer" title="Fokus-Timer">
...
function handleCompleteTask(category, index, event) {
  ...
  if (typeof rawTask === 'object' && rawTask.date) {
    taskText += ` (${formatTerminDate(rawTask.date, rawTask.time)})`;
  }
  ...
  // Reset step checks for the completed task
  if (state.completedSteps) {
    delete state.completedSteps[taskText];
  }
...
function handleDeleteTask(category, index, event) {
  ...
  if (taskText && state.completedSteps) {
    delete state.completedSteps[taskText];
  }

  saveState();
  showToast(currentLang === 'de' ? 'Aufgabe gelöscht' : 'Task deleted');
  renderApp();
  updateZenView();
  populateAdhdTaskSelect();
}
...
function handleRestoreDoneTask(doneIndex) {
  ...
  saveState();
  showToast(currentLang === 'de' ? 'Aufgabe wiederhergestellt' : 'Task restored');
  renderApp();
  updateZenView();
  populateAdhdTaskSelect();
}

// DYNAMIC HISTORIC UNDO / REDO
function handleUndo() {
...
function handleSaveJson() {
  ...
  a.href = url;
  a.download = `flow-plan-${state.lastDate}.json`;
  a.click();
  ...
  reader.onload = () => {
    try {
      ...
      if (imported && imported.items) {
        ...
        saveState();
        showToast(currentLang === 'de' ? 'Plan erfolgreich importiert!' : 'Plan imported successfully!');
        renderApp();
        populateAdhdTaskSelect();
      }
    } catch(err) {
...
function updateReportPanel() {
  ...
  // XP & Gamification Level Calculation
  ...
  if (badgeEl) badgeEl.innerText = `Lv.${userLevel}`;
  const titleEl = document.getElementById('user-level-title');
  if (titleEl) titleEl.innerText = userTitle;
  const xpTextEl = document.getElementById('user-xp-text');
  if (xpTextEl) xpTextEl.innerText = `${currentLevelXp} / 100 XP`;
  const xpBarEl = document.getElementById('user-xp-bar');
  if (xpBarEl) xpBarEl.style.width = `${currentLevelXp}%`;

  let totalPending = 0;
  ['daily', 'weekly', 'todo', 'occasionally', 'termine'].forEach(cat => {
    totalPending += (state.items[cat] || []).length;
  });
  ...
  const rateEl = document.getElementById('report-rate-pct');
  if (rateEl) rateEl.innerText = `${pct}%`;
  ...
  if (catBarsEl) {
    ...
    const catStats = [
      ...
      { id: 'occasionally', label: t('occasionally') }
    ];
  ...
  if (list) {
    ...
    if (filteredDone.length === 0) {
      ...
      filteredDone.slice().reverse().forEach(item => {
        ...
        div.className = 'p-2 bg-white/[0.02] border border-white/5 rounded-lg flex justify-between items-center text-gray-300 hover:bg-white/5 transition';
        const catLabel = t(item.origin) || item.origin;
        div.innerHTML = `
          <div class="flex items-center gap-1.5 overflow-hidden pr-2">
            <span class="text-[10px] px-1.5 py-0.5 rounded bg-purple-500/20 text-purple-300 border border-purple-500/30 shrink-0">${catLabel}</span>
            <span class="truncate font-medium text-xs text-white">${item.task}</span>
          </div>
          <span class="text-gray-500 font-mono text-[10px] shrink-0">${item.time || ''}</span>
        `;
...
function submitFeedback() {
  ...
  if (text.trim()) {
    showToast({
      de: 'Vielen Dank für dein Feedback! ❤️',
      en: 'Thank you so much for your feedback! ❤️',
      es: '¡Muchas gracias por tus comentarios! ❤️',
      el: 'Σας ευχαριστούμε πολύ για τα σχόλιά σας! ❤️'
    }[currentLang]);
  ...
  if (timerRunning) {
    ...
    timerInterval = setInterval(() => {
      if (timerSeconds > 0) {
        ...
        playProceduralSound();
        showToast({
          de: 'Fokus-Zeit abgelaufen! Zeit für eine Pause! ☕',
          en: 'Focus timer finished! Time for a short break! ☕',
...
function updateTimerDisplay() {
  ...
  if (zenDisp) zenDisp.innerText = str;
}
...
function updateZenView() {
  ...
  if (dailyTasks.length > 0) {
    chosen = dailyTasks[0];
  } else if (weeklyTasks.length > 0 || todoTasks.length > 0) {
    chosen = weeklyTasks[0] || todoTasks[0];
  } else if (occasionallyTasks.length > 0) {
    chosen = occasionallyTasks[0];
  }
  ...
  if (!chosen) {
    ...
    if (zenCatEl) zenCatEl.innerText = `${t('next_rec')} · ${catName}`;
...
function zenCompleteCurrentTask() {
  if (!currentZenTaskInfo) {
    showToast(currentLang === 'de' ? 'Keine aktive Aufgabe zum Erledigen.' : 'No active task.');
    return;
  }
...
function populateAdhdTaskSelect() {
  ...
  // Collect unique active tasks
  const allTasks = [];
  ['daily', 'weekly', 'todo', 'occasionally'].forEach(cat => {
    (state.items[cat] || []).forEach(task => {
      if (!allTasks.includes(task)) allTasks.push(task);
    });
  });

  // Also include translated versions of presets
  const standardPresetsInCurrentLang = [
    ...DEFAULT_TASKS_BY_LANG[currentLang].daily,
    ...DEFAULT_TASKS_BY_LANG[currentLang].weekly,
    ...DEFAULT_TASKS_BY_LANG[currentLang].occasionally
  ];

  standardPresetsInCurrentLang.forEach(task => {
    if (!allTasks.includes(task)) allTasks.push(task);
  });
...
function pickRandomTask() {
  ...
  if (dailyTasks.length > 0) {
    chosen = dailyTasks[Math.floor(Math.random() * dailyTasks.length)];
  } else {
    const mixedMidPriority = [];
    const maxLen = Math.max(todoTasks.length, weeklyTasks.length);
    for (let i = 0; i < maxLen; i++) {
      if (todoTasks[i]) mixedMidPriority.push(todoTasks[i]);
      if (weeklyTasks[i]) mixedMidPriority.push(weeklyTasks[i]);
    }

    if (mixedMidPriority.length > 0) {
      chosen = mixedMidPriority[Math.floor(Math.random() * Math.min(mixedMidPriority.length, 3))];
    } else if (occasionallyTasks.length > 0) {
      chosen = occasionallyTasks[Math.floor(Math.random() * occasionallyTasks.length)];
    }
  }
  ...
  if (!chosen) {
    ...
    const stepsBtnLabel = t('open_steps');
    ...
    box.innerHTML = `
      <div class="flex flex-col items-center gap-2 w-full py-1">
        ...
        <div class="flex flex-wrap items-center justify-center gap-2 mt-2 w-full">
          <button onclick="startTaskTimer('${chosen.task.replace(/'/g, "\\'")}')" class="px-3 py-1.5 bg-purple-500/20 hover:bg-purple-500/30 border border-purple-500/40 text-purple-300 rounded-lg text-xs font-semibold cursor-pointer transition flex items-center gap-1 hover:scale-105 active:scale-95">
            <i data-lucide="timer" class="w-3.5 h-3.5"></i>
            <span>Timer</span>
          </button>
          <button onclick="openTaskStepsModal('${chosen.cat}', ${taskIdx})" class="px-3 py-1.5 bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/40 text-amber-300 rounded-lg text-xs font-semibold cursor-pointer transition flex items-center gap-1">
            <i data-lucide="footprints" class="w-3.5 h-3.5"></i>
            <span>${stepsBtnLabel}</span>
          </button>
          <button onclick="handleCompleteTask('${chosen.cat}', ${taskIdx}); pickRandomTask();" class="px-3 py-1.5 bg-emerald-500/20 hover:bg-emerald-500/30 border border-emerald-500/40 text-emerald-300 rounded-lg text-xs font-semibold cursor-pointer transition flex items-center gap-1">
            <i data-lucide="check" class="w-3.5 h-3.5"></i>
...
function generateTaskSteps(specificTask) {
  ...
  // Map key to German equivalent to look up in the steps database
  const deKey = getGermanStandardKey(val);
  let steps = TASK_STEPS_DATABASE[deKey]?.[currentLang];

  // Smart fallback step generator for custom user tasks
  ...
  // Load saved steps checked state
  if (!state.completedSteps) state.completedSteps = {};
...
function toggleStepCheck(checkbox, stepIndex) {
  ...
  if (checkbox.checked) {
    ...
    if (!state.completedSteps[targetTask].includes(stepIndex)) {
  ...
  // Check if ALL steps in the list are checked off
  const resBox = document.getElementById('adhd-steps-result');
  ...
  if (checkboxes.length > 0 && checkboxes.every(cb => cb.checked)) {
    ...
    if (targetTask) {
      ...
      setTimeout(() => {
        closeAdhdModal();
        // Delete checked step memory for completed task
...
// SOUNDSCAPE WEB AUDIO SYNTHESIZER with soft Fades
let audioCtx = null;
...
function playAmbientSound(type) {
  initAudioContext();
  // Silently stop active ambient soundscape
  ...
  // Smoothly ramp volume up over 1.5 seconds
...
function stopAmbientSound(silent = false) {
  if (soundGainNode && audioCtx) {
    ...
    // Smoothly fade-out volume over 0.8 seconds
    ...
    // Terminate oscillator nodes after fade-out
    setTimeout(() => {
...
function updateSoundscapeUI() {
  ['rain', 'ocean', 'alpha', 'wind'].forEach(st => {
    ...
    if (btn) {
      if (st === currentSoundType) {
        btn.className = 'p-2 bg-blue-500/30 border border-blue-400 rounded-xl text-left text-xs text-white font-bold transition cursor-pointer flex items-center gap-2 shadow-sm animate-pulse';
      } else {
        btn.className = 'p-2 bg-white/5 hover:bg-blue-500/20 border border-white/10 rounded-xl text-left text-xs text-gray-200 font-semibold transition cursor-pointer flex items-center gap-2';
      }
    }
  });
